...
class User
{
     ...
     *      min = 5,
     *      max = 50,
     *      minMessage = "Le nom d'utlisateur' doit comporter au moins {{ limit }} caractères",
     *      maxMessage = "Le nom d'un utlisateur doit comporter au plus {{ limit }} caractères"
     * )
     */
    private $user_name;
    ...
    /**
     * @ORM\OneToMany(targetEntity=Affecter::class, mappedBy="nom_etudiant",cascade={"all"},orphanRemoval=true)
     */
    private $affecters;
    ...
    public function removeAffecter(Affecter $affecter): self
    {
        if ($this->affecters->removeElement($affecter)) {
            // set the owning side to null (unless already changed)
            if ($affecter->getNomEtudiant() === $this) {
                $affecter->setNomEtudiant(null);
            }
        }
